...
export default function Leaderboard({ theme = "dark" }) {
  ...
  const themeClasses = {
    ...
    inputText: isDark ? "text-black" : "text-gray-900",
    buttonBg: "bg-[#2ECC71]",
    buttonHoverBg: "hover:bg-[#28b263]",
    cardBg: isDark ? "bg-gray-100" : "bg-gray-50",
    cardText: isDark ? "text-" : "text-gray-900",
    shadow: isDark ? "shadow-[0_0_10px_rgba(46,204,113,0.3)]" : "shadow-md",
  };
  ...
  return (
    <div className={`${themeClasses.bg} min-h-screen p-5`}>
      {/* Header & Search Input */}
      <motion.div
        ref={searchRef}
        className={`mt-10 max-w-5xl mx-auto flex flex-wrap items-center justify-between mb-6 gap-4 
          ...
          >
            ...
            <div>
              ...
              <p className={`text-sm ${themeClasses.secondaryText}`}>
                Score: {searchedUser.score} | Repos: {searchedUser.repositories} | Commits: {searchedUser.commits}
              </p>
            </div>
          </motion.div>
        ) : searchTerm ? (
      ...
      {/* Users List */}
      <div className="max-w-5xl mx-auto mt-10 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
        {filteredSortedUsers.map((user) => (
          <motion.div
            key={user.username}
            className={`p-4 rounded-xl ${themeClasses.cardBg} ${themeClasses.shadow} transition-transform hover:scale-105`}
          ...
          >
            <div className="flex items-center gap-4 mb-3">
              <img
                src={user.avatar}
                alt={user.username}
                className="w-14 h-14 rounded-full border-2 border-[#2ECC71]"
              />
              <div>
                <h3 className={`text-xl font-semibold ${themeClasses.cardText}`}>
                  {user.username}
                </h3>
                <p className={`text-sm ${themeClasses.secondaryText}`}>
                  Score: {user.score}
            ...
            <div className="flex justify-around mt-2 text-[#2ECC71]">
              <div className="flex items-center gap-1 text-sm">
                <FaGithub /> {user.repositories}
              </div>
              <div className="flex items-center gap-1 text-sm">
                <FaCodeBranch /> {user.commits}
              </div>
              <div className="flex items-center gap-1 text-sm">
                <FaBug /> {user.issues}
              </div>
